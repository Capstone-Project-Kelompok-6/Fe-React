...
import { editOnlineClasses, editOnlineVideoClasses } from "../../../stores/features/onlineClassesSlice";
...
const ModalEditOnlineClasses = ({ handleModalEditTrigger, handleActionDropdown, update }) => {
	const { class_id, video_title, workout, video, video_name, price, description, workout_id } = update;
	...
	const handleUpdate = (e) => {
		...
		if (!errors.video) {
			const response = dispatch(editOnlineClasses({ class_id, video_title, workout_id, price, description })).then(() => {
				const video = formData.get("video");
				if (video.name !== "") {
					dispatch(editOnlineVideoClasses({ class_id, video, video_name }));
				}
			});
						...
						<form onSubmit={handleUpdate} className="rounded-xl bg-white shadow">
							...
							<div className="h-[65vh] overflow-y-auto p-6">
								<div className="h-[90&] space-y-6">
									...
									<div className="relative">
										<input type="number" min="1" id="price" name="price" className={inputNotError} placeholder=" " defaultValue={price} />
