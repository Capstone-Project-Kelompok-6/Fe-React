...
const InstructorList = () => {
	...
	useEffect(() => {
		if (debouncedKeyword) {
			InstructorAPI.searchInstructor(debouncedKeyword.toLowerCase()).then((result) => setInstructor({ status: true, data: result.data.data }));
		} else {
			setTimeout(
				() =>
					InstructorAPI.getInstructor(10).then((result) =>
						setInstructor({ status: true, data: result.data.data, page: result.data.data.page ? 1 : result.data.data.page })
					),
				1300
			);
		}
				...
				<div className="fixed left-0 right-0 z-20 w-full bg-white bg-opacity-90 px-6 py-2 shadow-3 backdrop-blur-sm">
					<div className="flex items-center space-x-4">
						<div className="min-w-0 flex-1">
							<h2 className="text-sm font-medium text-neutral-100-2 md:pl-52 md:text-lg">Manage Instructor</h2>
						</div>
					...
					<div className="w-full rounded-xl bg-white py-6 px-6 shadow-4">
						{instructor.status ? (
							<div>
										...
										<div className="w-full overflow-x-auto rounded-xl scrollbar-hide md:scrollbar-default">
											<table className="w-full text-left">
												...
												{instructor.data.rows?.map((item, i) => {
													return <InstructorListItem key={item.instructor_id} data={{ ...item, no: instructor.data.page * 10 - 10 + 1 + i }} />;
												})}
											</table>
										</div>
										<nav className="flex flex-col items-center justify-between pt-4 md:flex-row lg:flex-row xl:flex-row">
											...
											<ul className="mt-3 inline-flex items-center -space-x-px text-sm md:mt-0 lg:mt-0 xl:mt-0">
												<li>
													<button
														type="button"
														className={instructor.data.page === 1 ? previousButtonDisabled : previousButtonActive}
												...
												<li>
													<button
														type="button"
														className={instructor.data.page + 1 > instructor.data.total_pages ? nextButtonDisabled : nextButtonActive}
								...
								{modalCreateTrigger && <ModalCreateInstructor handleModalCreateTrigger={handleModalCreateTrigger} />}
							</div>
						) : (
