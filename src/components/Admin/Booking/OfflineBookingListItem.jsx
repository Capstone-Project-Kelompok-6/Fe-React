...
const OfflineBookingListItem = ({ data }) => {
	const { book_id, class_dates, email, full_name, image_name, instructor_name, price, workout, workout_image, created_at } = data;
						...
						setTimeout(
							() =>
								Swal.fire({
									icon: "success",
									title: "Deleted",
									text: "Offline booking data successfully deleted",
		...
		<div>
			<div className="relative h-full rounded-20 border bg-white py-4 shadow-4">
							...
							<div>
								...
								<div className="absolute right-0 z-40 mr-6 mt-5 w-32 rounded-xl bg-white shadow-4 transition duration-300">
									<ul className="list-reset">
										...
										<li>
											<button type="submit" className={`rounded-b-xl hover:rounded-b-xl ${actionDropdownDelete}`} onClick={handleDelete}>
				...
				<div className="flex flex-col px-5 md:flex-row md:items-center">
					<img className="my-2 h-48 w-full rounded-xl object-cover object-center md:h-[190px] md:w-32" src={workout_image} alt={image_name} />
					<div className="flex flex-col py-0 px-0 leading-normal md:py-2 md:px-4">
						<h5 className="mb-2 mt-4 text-base font-semibold tracking-tight text-neutral-100-2 md:mt-2">{workout}</h5>
						...
						<div className="mt-4 grid grid-cols-1 gap-3 md:mt-10 md:grid-cols-2 md:gap-12 xl:grid-cols-2">
							<div className="flex">
								...
								<div className="flex-1 leading-relaxed">
									<p className="text-sm font-medium tracking-tight text-neutral-100-2">{full_name}</p>
